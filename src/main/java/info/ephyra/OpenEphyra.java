...
public class OpenEphyra
{
    ...
    public static final String NORMALIZER = "resources/scorenormalization/classifiers/"
        + "AdaBoost70_"
        + "Score+Extractors_"
    ...
    public OpenEphyra(String dir)
    {
        ...
        if (!OpenNLP.createTokenizer(dir
            + "resources/nlp/tokenizer/opennlp/EnglishTok.bin.gz"))
        ...
        if (!OpenNLP.createSentenceDetector(dir
            + "resources/nlp/sentencedetector/opennlp/EnglishSD.bin.gz"))
        ...
        if (!OpenNLP.createPosTagger(dir
            + "resources/nlp/postagger/opennlp/tag.bin.gz", dir
            + "resources/nlp/postagger/opennlp/tagdict"))
            MsgPrinter.printErrorMsg("Could not create OpenNLP POS tagger.");
        // if (!StanfordPosTagger.init(dir + "resources/nlp/postagger/stanford/" +
        ...
        if (!OpenNLP.createChunker(dir
            + "resources/nlp/phrasechunker/opennlp/EnglishChunk.bin.gz"))
            MsgPrinter.printErrorMsg("Could not create chunker.");

        // create syntactic parser
        MsgPrinter.printStatusMsg("Creating syntactic parser...");
        // if (!OpenNLP.createParser(dir + "resources/nlp/syntacticparser/opennlp/"))
        ...
        NETagger.loadListTaggers(dir + "resources/nlp/netagger/lists/");
        NETagger.loadRegExTaggers(dir + "resources/nlp/netagger/patterns.lst");
        MsgPrinter.printStatusMsg("  ...loading models");
        // if (!NETagger.loadNameFinders(dir + "resources/nlp/netagger/opennlp/"))
        ...
        // if (!OpenNLP.createLinker(dir + "resources/nlp/corefresolver/opennlp/"))
        ...
        if (!WordNet.initialize(dir
            + "resources/ontologies/wordnet/file_properties.xml"))
        ...
        if (!FunctionWords.loadIndex(dir
            + "resources/indices/functionwords_nonumbers"))
            MsgPrinter.printErrorMsg("Could not load function words.");

        // load prepositions
        MsgPrinter.printStatusMsg("Loading prepositions...");
        if (!Prepositions.loadIndex(dir + "resources/indices/prepositions"))
        ...
        if (!IrregularVerbs.loadVerbs(dir + "resources/indices/irregularverbs"))
            MsgPrinter.printErrorMsg("Could not load irregular verbs.");

        // load word frequencies
        MsgPrinter.printStatusMsg("Loading word frequencies...");
        if (!WordFrequencies.loadIndex(dir + "resources/indices/wordfrequencies"))
        ...
        // "resources/answertypes/patterns/answertypepatterns"))
        ...
        if (!QuestionInterpreter.loadPatterns(dir
            + "resources/patternlearning/questionpatterns/"))
        ...
        if (!AnswerPatternFilter.loadPatterns(dir
            + "resources/patternlearning/answerpatterns/"))
    ...
    protected void initFactoid()
    {
        ...
        //Search.addKnowledgeMiner(new BingAzureKM());  // todo: broken
