...
public class PatternLearner {
	...
	public static void init() {
		...
		if (!OpenNLP.createTokenizer("resources/nlp/tokenizer/opennlp/" +
									 "EnglishTok.bin.gz"))
...
//		LingPipe.createTokenizer();
		...
		if (!OpenNLP.createSentenceDetector("resources/nlp/sentencedetector/" +
...
//		LingPipe.createSentenceDetector();
		...
		if (!OpenNLP.createPosTagger("resources/nlp/postagger/opennlp/tag.bin.gz",
									 "resources/nlp/postagger/opennlp/tagdict"))
			MsgPrinter.printErrorMsg("Could not create OpenNLP POS tagger.");
//		if (!StanfordPosTagger.init("resources/nlp/postagger/stanford/" +
//				"train-wsj-0-18.holder"))
//			MsgPrinter.printErrorMsg("Could not create Stanford POS tagger.");
		
		// create chunker
		MsgPrinter.printStatusMsg("Creating chunker...");
		if (!OpenNLP.createChunker("resources/nlp/phrasechunker/opennlp/" +
								   "EnglishChunk.bin.gz"))
			MsgPrinter.printErrorMsg("Could not create chunker.");
		
		// create syntactic parser
//		MsgPrinter.printStatusMsg("Creating syntactic parser...");
//		if (!OpenNLP.createParser("resources/nlp/syntacticparser/opennlp/"))
...
//		}
		...
		NETagger.loadListTaggers("resources/nlp/netagger/lists/");
		NETagger.loadRegExTaggers("resources/nlp/netagger/patterns.lst");
		MsgPrinter.printStatusMsg("  ...loading models");
//		if (!NETagger.loadNameFinders("resources/nlp/netagger/opennlp/"))
...
//		if (!OpenNLP.createLinker("resources/nlp/corefresolver/opennlp/"))
//			MsgPrinter.printErrorMsg("Could not create linker.");
		...
		if (!WordNet.initialize("resources/ontologies/wordnet/file_properties.xml"))
		...
		if (!FunctionWords.loadIndex("resources/indices/functionwords_nonumbers"))
			MsgPrinter.printErrorMsg("Could not load function words.");
		
		// load prepositions
		MsgPrinter.printStatusMsg("Loading prepositions...");
		if (!Prepositions.loadIndex("resources/indices/prepositions"))
		...
		if (!IrregularVerbs.loadVerbs("resources/indices/irregularverbs"))
		...
		if (!QuestionInterpreter.loadPatterns("resources/patternlearning/" +
											  "questionpatterns/"))
...
//			Search.addKnowledgeMiner(new IndriKM(indriServers, true));
	...
	public static boolean interpret(String qFile, String aFile, String pFile) {
		...
		return interpretQuestions("resources/patternlearning/interpretations");
	}
	...
	public static boolean extract() {
		...
		queries = formQueries("resources/patternlearning/interpretations");
		for (Query query : queries) queryList.add(query);
		queries = formQueries("resources/patternlearning/interpretations_extract");
		for (Query query : queries) queryList.add(query);
		...
		return savePatterns("resources/patternlearning/answerpatterns_extract");
	}
	...
	public static boolean assess() {
		...
		if (!loadPatterns("resources/patternlearning/answerpatterns_extract"))
			return false;
		...
		queries = formQueries("resources/patternlearning/interpretations");
		for (Query query : queries) queryList.add(query);
		queries = formQueries("resources/patternlearning/interpretations_assess");
		for (Query query : queries) queryList.add(query);
		queries = queryList.toArray(new Query[queryList.size()]);
		...
		// save answer patterns
		MsgPrinter.printSavingPatterns();
		return savePatterns("resources/patternlearning/answerpatterns_assess");
	}
	...
	public static boolean filter() {
		// load answer patterns
		MsgPrinter.printLoadingPatterns();
		if (!loadPatterns("resources/patternlearning/answerpatterns_assess"))
			return false;
		...
		// save answer patterns
		MsgPrinter.printSavingPatterns();
		return savePatterns("resources/patternlearning/answerpatterns");
	}
